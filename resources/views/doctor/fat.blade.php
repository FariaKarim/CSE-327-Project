...
<script type="text/javascript">
  function calcFat()
  {
      ...
      if(opW=="kg"&&opH=="m"&&opG=="male"&&age<18)
      {
         var bmi=weight/(height*height);
          document.getElementById("fat").value=(1.51*bmi)-(0.70*age)-2.2;
      }
      else if(opW=="kg"&&opH=="m"&&opG=="female"&&age<18)
      {

        var bmi=weight/(height*height);
          
          document.getElementById("fat").value=(1.51*bmi)-(0.70*age)-1.4;
      }
      else if(opW=="lbs"&&opH=="inch"&&opG=="male"&&age<18)
      {
        var bmi=(weight*703)/(height*height);
       
          document.getElementById("fat").value=(1.51*bmi)-(0.70*age)-2.2;
      }
      else if(opW=="lbs"&&opH=="inch"&&opG=="female"&&age<18)
      {

        var bmi=(weight*703)/(height*height);
          document.getElementById("fat").value=(1.51*bmi)-(0.70*age)-1.4;
      }
      else if(opW=="kg"&&opH=="m"&&opG=="male"&&age>=18)
      {
         var bmi=weight/(height*height);
          document.getElementById("fat").value=(1.20*bmi)+(0.23*age)-16.2;
      }
      else if(opW=="kg"&&opH=="m"&&opG=="female"&&age>=18)
      {
         var bmi=weight/(height*height);
         document.getElementById("fat").value=(1.20*bmi)+(0.23*age)-5.4;
      }
      else if(opW=="lbs"&&opH=="inch"&&opG=="male"&&age>=18)
      {
        var bmi=(weight*703)/(height*height);
      ...
      else
      {
        ...
        document.getElementById("fat").value="Error in selecting units";
      }
...
<div class="vertical-menu5">
    <a href="" class="active">Body Fat</a>
</div>
...
<div class="container">
  ...
  <br>
  <div class="row">
     <button type="button" class="btn btn-danger btn-lg" style="background-color: #FF4B4B; margin-left: 650px" onclick="calcFat()">Calculate</button>
  </div>
  <label for="Gender" style="font-size:20px;color:#FF4B4B">Body water:</label><input type="text" id="fat">&nbsp<label for="Gender" style="font-size:20px;color:#FF4B4B">%</label>
  </form>
</div>
 </div>
</div>
